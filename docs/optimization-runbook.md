...
| --- | --- | --- | --- | --- | --- | --- |
| EXP-017 | 2026-07-22 | Re-validate the memory layer's only accuracy-positive result (+13.3pp on plain-multihop, v13/llama-server era) under the current stack (qwen3.6-35b on vLLM), correct-only distillation, learn_from_failure OFF - the last open claim to memory upside after EXP-016. First run WITH per-task injection stats (prehend `d25b2b6` / rlm-trainer `aaf8571`). | None (eval only). `memory_cold_warm.py`, tasks/multihop.json (60 tasks, 5 distinct questions, ctx ~315k chars), fresh bank `runs/banks/exp017-plainmultihop`. | Cold/warm compare + the new injection-overlap analysis: which flipped tasks actually received injections. | Cold **80.0%** (48/60, 0 infra) - qwen already beats v13's cold (76.7/81.7%) on this set. Warm 76.7%, delta -3.3pp, flips 9 up : 11 down. **The injection stats settle attribution: only 5/60 warm tasks received ANY injection (the 5 whose documents match the 5 bank entries' ctx_sigs), all 5 were correct anyway, and the overlap between injected tasks and flipped tasks is EMPTY - both flip sets lie entirely on zero-injection tasks.** The -3.3pp is run-to-run variance, not memory harm; but the upside is exactly zero. Mechanism: the ctx-sig gate (devils-advocate rec #2) correctly restricts retrieval to same-document experiences, and this task set has 60 distinct documents - so memory structurally cannot generalize here. The historical +13.3pp predates honest ctx-sig gating and is now best read as the cross-document self-injection rec #2 called illusory. | **Memory upside NOT reproduced; attribution clean thanks to the new stats.** Standing conclusion across EXP-016+017: under the current honest configuration the memory layer helps only when the same document recurs across tasks - and on the one set with that property (the clustered set) it HURT. Park the memory layer: no default-on, no further tuning without a workload where document recurrence is real (e.g. the librarian's production query stream, not synthetic task sets). |
| EXP-016 | 2026-07-21 | **QUEUED - memory contrastive A/B.** The contrastive failure channel (negative guard rules from wrong solves, prehend ADR-0011/0020) has never been validated: plain-multihop had ~5 distinct questions (entry-id collapse), and the rec-#3 coldwarm set is anti-contrastive (one task per doc + ctx-sig gate means a learned negative can never be retrieved again). rlm-trainer `tasks/contrastive/cluster_tasks.json` (built `19f172a`) fixes the shape: 160 unique questions in 40 doc-clusters of 4, ctx-sig identical within a cluster so failure-learned guards CAN fire for sibling questions. | None yet. Planned: `memory_cold_warm.py --tasks tasks/contrastive/cluster_tasks.json`, clean vs `--learn-from-failure`, same bank-dir discipline as ADR-0022. | Cold/warm accuracy delta per arm + bank composition audit (negatives actually written AND retrieved); strict scorer. | Ran 2026-07-21/22, `runs/exp016-{clean,lff}/`, 4 phases x 160 tasks, qwen3.6-35b on :8080. Cold parity holds (112 vs 114 correct - arms comparable). **Within-arm warm deltas: clean -3.1pp (flips 17 up / 22 down, sign p=0.52, n.s.); lff -8.1pp (9 up / 22 down, sign p=0.029, SIGNIFICANT).** Cross-arm warm-vs-warm: clean ahead 21-15, p=0.41, n.s. Bank audit: the channel MECHANICALLY works - lff bank 102 negatives vs clean's 44 (clean's negatives are distiller-labeled cautions from correct solves; lff adds forced negatives from wrong solves), 181 vs 129 entries. Warm also degrades ops in both arms: infra_fail 6->8 (clean) / 5->12 (lff), avg latency +27% / +45%. Caveat: per-task injection counts are not persisted in run artifacts, so "negatives retrieved" is inferred from the deltas, not directly counted. | **CHANNEL VALIDATED AS TESTABLE, RESULT NEGATIVE.** Failure-distilled negatives make warm significantly worse than its own cold; correct-only warm is flat-to-slightly-negative on this set. Consistent with the kb-saturation prior: at 70% cold accuracy over corpus tasks, injected guidance adds prompt mass and failure modes, not signal. Do NOT enable learn_from_failure by default. Open follow-ups, in order of value: (a) persist per-task injection stats so retrieval of negatives is observable, not inferred; (b) re-test memory upside on plain large-context tasks under the vLLM stack (where +13.3pp was measured) before concluding memory itself is net-neutral; (c) negative-injection cap tuning is pointless until (a) exists. |
| EXP-015 | 2026-07-21 | Re-test of the dense fusion arm after two changes EXP-013 predates: (a) the decorrelated 188-ask bakeoff (rlm-trainer bench-librarian-vs-lightrag addenda 2-4) showed the ask set's lexical bias was masking dense's value - fused(dense+BM25) >= max(arms) at every depth, and local bge-m3 strictly dominates the LightRAG arm; (b) retrieval-first briefing K=12 + cite-only-note are now merged, which may cap the docs-read blowup (14.7 -> 24.5) that EXP-013 blamed for converting a +5.5pp retrieval win into a -5.4pp citation loss. **CAVEAT logged upfront: running at `KB_RETRIEVAL_TOP_K=19` (the wedge-debug setting, not prod 8), so this measures dense@19 on the NEW briefing stack - close kin to the config EXP-013 rolled back.** | `KB_DENSE_ENABLED=true` (knowledge-base `.env.librarian`, 2026-07-21), librarian restarted, dense_ready confirmed. | e2e gate `kb_ask_eval.py`, v2 asks, 3 reps (`runs/qwen36-90ask-v2-densearm/`), paired vs the 07-18 combined-arm baseline. Per EXP-012's power finding, treat any sub-4pp delta as unresolved. | **EXP-013 REPRODUCED, on the new briefing stack and with significance this time.** 270/270 rows, 0 infra fails. vs combined baseline: gold_exact 75.0 -> 65.8 (**-9.2pp, CI [1.7, 17.5], SIGNIFICANT** - clears even the gate's blunt power), gold_recall 89.2 -> 84.2 (-5.0 n.s., same magnitude and sign as EXP-013's -5.1), answer_match flat (0.0 [-4.6, +4.6]), citation precision 0.824 -> 0.743, p90 111.8 -> 142.2s, median 35.7 -> 38.4s. The retrieval-first briefing did NOT rescue depth 19. | **ROLLED BACK** same day (`.env.librarian` KB_DENSE_ENABLED=false, librarian restarted, health confirmed). Now TWO e2e falsifications of "retrieval recall proxies citation at k=19" - the 188-ask bakeoff (addenda 2-4) remains true as a retrieval measurement, but the model cannot exploit a deeper candidate list. Untested remainder: dense at prod depth **k=8** (never gated; EXP-013's instrument said hybrid LOSES at k=8, so only worth a gate if the instrument's known mispredictions cut both ways). |
| EXP-014 | 2026-07-13 | local-ai#6: the p90 tail (~97s) survived the retrieval fix, so grep-flailing was never what dominates it. Four suspects were on the table (sub-call batch contention, oversized-part prefill, repeat-guard retries, KV preemption). Test them against the rows we already have. | None - analysis of `runs/qwen36-50ask-bm25-stem-20260712/` (the issue correctly predicted no new instrumentation was needed). | Decile decomposition of `elapsed_s` against `output_tokens` / `prompt_tokens` / `decode_tok_s`, then split by ask coverage. n=150 rows. | **The tail is the REFUSAL path.** `corr(output_tokens, elapsed_s) = 0.995`; the slowest decile emits 9.8x the tokens and takes 10.1x the time. **`decode_tok_s` is FLAT across deciles (196.8 / 187.9 / 205.8)** - the server is not slower for slow asks, which falsifies contention, preemption and KV pressure outright; 0 rows degenerate. Split by coverage: covered median 20.2s / p90 **55.2s**; uncovered median 58.2s / p90 **176.0s**, reading 29.5 docs vs 9.3. **Uncovered asks are 20% of rows but 42% of wall-clock.** When the corpus cannot answer, the RLM searches exhaustively before concluding it cannot - which is exactly why better ranking never moved the tail. | **inconclusive as a fix, decisive as a diagnosis.** Cause named and the serving-side suspects are dead. The lever is a search budget on the give-up path, same surface as the refusal work (knowledge-base#7), NOT a serving change. Posted to local-ai#6. n=30 uncovered rows is thin; v2 now carries 65 uncovered asks, so the next gate resolves it properly. |
| EXP-013 | 2026-07-13 | knowledge-base#6: BM25 collapses when a question stops echoing the document (recall@1 72.2% verbatim -> 40.0% paraphrase), which is how real agents ask. A dense arm (local bge-m3 on :8084, the model LightRAG pays DeepInfra for) fused by RRF should recover the paraphrase side without giving up the lexical side. | `librarian/dense.py` (dense index + RRF); `category_search` fuses BM25 with dense at doc level. `KB_DENSE_ENABLED=true`, `KB_RETRIEVAL_TOP_K=19`. Ships OFF by default. | Retrieval instrument (`.autoresearch/retrieval/evaluate.py`, n=261 covered v2 asks, scores `category_search` itself, CPU-only). Then the e2e gate (`kb_ask_eval.py`, v2, reps 2, n=652). | **Retrieval said yes. The gate said no, and that gap IS the finding.** Instrument: hybrid wins both styles at k=19 (verbatim 96.7 -> 97.4, paraphrase 84.5 -> 90.0; mean 90.62 -> 93.68) and LOSES at k=8 (85.20 < 86.65). **e2e gate (n=494 paired covered rows, identical ask+rep): hybrid+k19 is WORSE on everything.** gold_recall 75.1 -> 70.0 (**-5.1**), verbatim 80.6 -> 75.8 (-4.8), paraphrase 67.3 -> 62.0 (-5.4), p90 89.5 -> 101.9s (**+12.4**), docs read/ask 14.7 -> 24.5 (**+9.8**). Paraphrase sign test at ask level: B wins 14, A wins 24, **p=0.14 (not significant)** - so this is "no benefit at real cost", NOT a catastrophic subgroup regression (an earlier n=356 read showed -11.0pp paraphrase; it did not survive more rows, and I should not have called it a headline). | **ROLLED BACK.** Arm B killed at 495/652 rows once the direction was unambiguous; `.env.librarian` reverted to shipped. **A +5.5pp retrieval-recall gain converted to a -5.4pp citation LOSS.** Deepening the candidate list 8 -> 19 makes the model read ~10 more docs and cite the gold one slightly less: it is a bad reranker, and more options make it worse. The dense arm is not exonerated but is not separately indicted either - it never got to pay off, because the depth it needs is what breaks the model. **Retrieval recall is not a valid proxy for e2e citation past k=8 on this stack.** The autoresearch instrument (`.autoresearch/retrieval/evaluate.py`) is now known to mispredict; see its "what this cannot resolve" section, which called this exact failure and was right. |
| EXP-012 | 2026-07-13 | Oversized docs are served as parts (002.1, 002.2, ...) and BM25 scores each part independently, so a many-part doc should be able to crowd the candidate list with parts of ITSELF - spending slots that never show the model another document. | `librarian/corpus.py::_one_row_per_doc` - `category_search` returns one row per DOC, at that doc's best-scoring part. No embeddings, no new dependency. Companion part ids stay in each part's summary, so nothing becomes unreachable. | Same retrieval instrument (n=261). Measured the waste directly first: mean **4.0 of 19** candidate slots spent on repeat parts of one doc, on **248 of 261** asks; worst case **17 of 19**. | **Both styles up, for free: verbatim 94.0 -> 96.0, paraphrase 74.5 -> 77.3 at the shipped depth 8** (mean 84.29 -> 86.65). Raising depth to 19 on top: 96.7 / 84.5 (mean 90.62). **This beat adding a dense retriever at the shipped depth** (86.65 > 85.20) - the largest single win in the sequence needed no embeddings at all. | **DISCARD - the gate found NOTHING (2026-07-14, arm C complete, n=652).** gold_recall 75.3 -> 74.3, gold_exact 59.2 -> 57.5, answer_match 83.3 -> 81.2, no_citation 6.7 -> **9.4**, p90 114.9 -> 124.3s. Every point estimate favours baseline, and **the paired bootstrap clears none of them**: recall diff 1.0 [-3.0, 4.9], exact 1.7 [-2.3, 5.8], answer_match 2.2 [-1.3, 5.7] (n=261 asks, 153 clusters). So dedup is **flat, not worse** - but a +2.4pp instrument win produced **zero measurable product gain**, and by the program's own "ties go to the simpler code" rule that is a discard, not a keep. **The instrument has now mispredicted on BOTH of its promotions** (EXP-013 hybrid+k19, and this): its predictive validity against the gate is 0 for 2. *(History: originally folded into arm B; gated alone after EXP-013 rolled back. A dedup-specific mechanism had been FALSIFIED as the cause of EXP-013's regression - damage was -8.2 on single-part golds, which dedup cannot orphan, vs -2.0 on multi-part - which is why it survived to its own arm.)* **THE REAL FINDING IS THE GATE'S POWER.** Its 95% CI is ~±4pp while the retrieval instrument proposes 2-5pp candidates, so **the confirmation test cannot resolve most of what the loop generates** - we have been promoting changes the gate is arithmetically incapable of adjudicating, and "GATE-FAIL" vs "gate-flat" has been partly a coin flip. EXP-013's -5.1pp rollback sits right at that resolution limit and was never significance-tested (its run died at 495/652 rows, so it cannot be rescored). Filed as local-ai#8. Fix the power before running another arm. |
| EXP-011 | 2026-07-10 | Validate the bf16 KV revert (ADR-0016) end-to-end: does the committed FlashInfer + W8 + bf16 stack regress grounding vs prior arms, and what is its real-workload performance? | No config change (validation of the deployed stack). | 6-gold gate (n=18) + full 50-ask gate (n=150, 40 covered + 10 uncovered) through the real librarian `:8800`, gold-rescored (`score_gold_accuracy.py`); decode throughput bench (`concurrent_decode.py`); vs Jul-9 pre-campaign 50-ask baseline rescored identically. | **Grounding unregressed on the load-bearing metrics.** 6-gold: recall 94.4%, answer_match 94.4%, 0 fab/infra/degen. 50-ask: **recall 79.2% == Jul-9 baseline**, answer_match 90.8% (>= 89.7%), no_citation 4.2%. **Performance**: decode **+20% @56k N=4** (674 vs 563, reproduced 3rd time); end-to-end 50-ask latency **~2x faster than pre-campaign** (median 31.3 vs 61.8 s, mean 51 vs 136, p90 124 vs 431) - but that is the whole campaign incl. the EXP-002/003/004 tail-cutting prompt fixes, not the KV flip. **Soft spot**: precision 0.72 vs baseline 0.784 (over-citation - recall flat, extra docs cited), campaign-wide not KV-attributable. | **bf16 KEPT; ADR-0016 flipped provisional -> accepted.** Over-citation precision filed as local-ai issue #5 (separate prompt/retrieval concern). |
| EXP-010 | 2026-07-10 | EXP-009 showed fp8 KV was a decode wash at 8k concurrency; does it *lose* at 56k (where KV is ~31% of decode bytes), and is fp8 ever worth its capacity? Clean 2x2 `{NHD,HND} x {fp8,bf16}` at the operating point. | Test: 3-config sweep + restore (`scratchpad/ab_sweep3.sh`), no prod change. Then **prod change**: dropped `--kv-cache-dtype fp8_e4m3` (bf16 default; local-ai `scripts/localai-vllm.service`, keeps FLASHINFER). | `concurrent_decode.py` aggregate tok/s @8k+56k N=1/2/4/8; Prometheus `vllm_kv_cache_usage_perc` for the capacity question. | **bf16 FASTER than fp8 @56k: +19% N4 (670.8 vs 562.9), +7% N2** - reproduced across two independent sweeps (EXP-009 gave +21.7% at N4); also +10% @8k N8. fp8's per-read dequant goes net-negative once a batch amortizes the bandwidth saving; its batch-1 +7.5% (ADR-0015) was the unrepresentative corner. **HND layout helps fp8 ~5% @56k but is a wash for bf16** (only ever patched fp8's dequant penalty). **fp8's capacity is unused**: peak real KV ~192k tokens (architectural) / ~56k observed (Prometheus, undersampled) << bf16's 245,760 pool. | **fp8 REVERTED to bf16** (local-ai ADR-0016 supersedes 0015; provisional, under prod observation). W8 head + FlashInfer stay. HND not adopted. Calibrated-fp8 follow-up (issue #3) now moot. |
| EXP-009 | 2026-07-10 | The adopted decode levers (W8 head, fp8 KV) were only ever measured at batch 1; at the real operating point (median batch 2, bursts to 16 - see local-ai `docs/flashinfer-tuning.md` concurrency warning) the sheet *predicted* W8's win shrinks ~1/B and fp8 holds. Isolate each at real concurrency. | No prod change. Three-config A/B (`scratchpad/ab_conc.sh` + `concurrent_decode.py`), one restart each, shared 8k prefix, 512 greedy `ignore_eos`: config1 W8+fp8 (baseline), config2 bf16-head+fp8 (isolate W8), config3 W8+bf16-KV (isolate fp8); known-good restored after. | Server decode timer, aggregate + per-request tok/s at N=1/2/4/8 (the fixed-prompt decode regime extended to real concurrency; this is what the levers were never measured in). | **W8 isolated: +6.7 / +5.8 / +5.7 / +2.9% at N1/2/4/8** - does NOT shrink ~1/B; holds ~6% through N4 (the Map point), fades to +3% only at N8, and buys a +9.5% larger KV pool (467,407 vs 426,602 tok) by freeing head VRAM. **fp8 isolated: decode WASH at 8k every N (-0.2 / +1.4 / +0.2 / -0.4%)** - NOT a per-sequence decode-speed lever; its value is **1.9x KV pool capacity** (467,407 vs 245,760 tok). Falsifies the sheet's transfer prediction AND ADR-0015's batch-1 "-15% weighted decode" (an artifact that did not survive) + "pool moot at concurrency 1" (the pool IS the benefit). fp8 decode @56k untested - KV bandwidth is a larger share there and is the only place a speed gain could still appear. | **both KEPT, rationale corrected** - W8 for ~6% decode + bigger pool; fp8 for 2x capacity, not speed. No config change (already the prod stack). ADR-0015 needs a superseding ADR for the corrected fp8 rationale. |
