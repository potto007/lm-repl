...
| EXP-012 | 2026-07-13 | Oversized docs are served as parts (002.1, 002.2, ...) and BM25 scores each part independently, so a many-part doc should be able to crowd the candidate list with parts of ITSELF - spending slots that never show the model another document. | `librarian/corpus.py::_one_row_per_doc` - `category_search` returns one row per DOC, at that doc's best-scoring part. No embeddings, no new dependency. Companion part ids stay in each part's summary, so nothing becomes unreachable. | Same retrieval instrument (n=261). Measured the waste directly first: mean **4.0 of 19** candidate slots spent on repeat parts of one doc, on **248 of 261** asks; worst case **17 of 19**. | **Both styles up, for free: verbatim 94.0 -> 96.0, paraphrase 74.5 -> 77.3 at the shipped depth 8** (mean 84.29 -> 86.65). Raising depth to 19 on top: 96.7 / 84.5 (mean 90.62). **This beat adding a dense retriever at the shipped depth** (86.65 > 85.20) - the largest single win in the sequence needed no embeddings at all. | **UNDER GATE (arm C, `runs/qwen36-v2-dedup-k8-20260713/`).** Originally folded into arm B; after EXP-013 rolled back, dedup is being gated ALONE at the shipped depth - one variable off shipped, no embeddings, no new dependency. This is the only retrieval change still standing. **It is NOT assumed to work:** EXP-013 just proved retrieval recall does not predict e2e citation, and dedup was scored on that same discredited instrument. A dedup-specific mechanism was tested and FALSIFIED as the cause of EXP-013's regression (damage was -8.2 on single-part golds, which dedup cannot orphan, vs -2.0 on multi-part golds), which is why it survived to its own arm rather than being rolled back with the rest. |
