...
| EXP-013 | 2026-07-13 | knowledge-base#6: BM25 collapses when a question stops echoing the document (recall@1 72.2% verbatim -> 40.0% paraphrase), which is how real agents ask. A dense arm (local bge-m3 on :8084, the model LightRAG pays DeepInfra for) fused by RRF should recover the paraphrase side without giving up the lexical side. | `librarian/dense.py` (dense index + RRF); `category_search` fuses BM25 with dense at doc level. `KB_DENSE_ENABLED=true`, `KB_RETRIEVAL_TOP_K=19`. Ships OFF by default. | Retrieval instrument (`.autoresearch/retrieval/evaluate.py`, n=261 covered v2 asks, scores `category_search` itself, CPU-only). Then the e2e gate (`kb_ask_eval.py`, v2, reps 2, n=652). | **Retrieval: hybrid wins BOTH styles at k=19 - verbatim 96.7 -> 97.4, paraphrase 84.5 -> 90.0** (mean 90.62 -> 93.68) vs BM25+dedup. **But it LOSES at k=8 (85.20 < 86.65)**: fusion buys nothing if the fused ranking is then truncated above where the fused hits land. Depth is a precondition for the dense arm, not an independent knob. e2e gate: **PENDING** (arm B, this run). | **PENDING e2e gate.** Retrieval win promoted, not shipped: recall says the gold doc reached the candidate list, not that the model read, cited, or answered from it - and not what it cost in latency (local-ai#6 tail is open). |
| EXP-012 | 2026-07-13 | Oversized docs are served as parts (002.1, 002.2, ...) and BM25 scores each part independently, so a many-part doc should be able to crowd the candidate list with parts of ITSELF - spending slots that never show the model another document. | `librarian/corpus.py::_one_row_per_doc` - `category_search` returns one row per DOC, at that doc's best-scoring part. No embeddings, no new dependency. Companion part ids stay in each part's summary, so nothing becomes unreachable. | Same retrieval instrument (n=261). Measured the waste directly first: mean **4.0 of 19** candidate slots spent on repeat parts of one doc, on **248 of 261** asks; worst case **17 of 19**. | **Both styles up, for free: verbatim 94.0 -> 96.0, paraphrase 74.5 -> 77.3 at the shipped depth 8** (mean 84.29 -> 86.65). Raising depth to 19 on top: 96.7 / 84.5 (mean 90.62). **This beat adding a dense retriever at the shipped depth** (86.65 > 85.20) - the largest single win in the sequence needed no embeddings at all. | **kept.** Folded into the arm-B gate with EXP-013 (they are not separable e2e in one night on one GPU; the instrument decomposes them). |
